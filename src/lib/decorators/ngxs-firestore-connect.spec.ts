...
describe('NgxsFirestoreConnect', () => {
    ...
    class TestState {
        constructor(private fs: TestFirestore) {}
    ...
    beforeEach(() => {
        ...
        TestBed.configureTestingModule({
            ...
            providers: [{ provide: TestFirestore, useValue: testFirestoreMock() }]
        }).compileComponents();
    ...
    it('EMIT action completes first', fakeAsync(() => {
        ...
        actions
            .pipe(ofActionCompleted(GetTestData, GetTestDataEmit))
            .subscribe((actionContext) =>
                actionsCompleted.push(getActionTypeFromInstance(actionContext.action.constructor))
            );
    ...
    it('keeps listening and updates store on every new emission', fakeAsync(() => {
        ...
        actions.pipe(ofActionCompleted(GetTestDataEmit)).subscribe((actionContext) => actionsCompleted++);
    ...
    it('action completes after first emission', fakeAsync(() => {
        ...
        collectionFn.mockReturnValue($);
        let completed = false;
        actions.pipe(ofActionCompleted(GetTestData)).subscribe((_) => {
            completed = true;
        });
    ...
    it('connects only ONCE when action is dispatched multiple times', fakeAsync(() => {
        ...
        collectionFn.mockReturnValue($);
        let count = 0;
        actions.pipe(ofActionCompleted(GetTestDataEmit)).subscribe((_) => count++);
