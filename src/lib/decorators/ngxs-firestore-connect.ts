...
export function NgxsFirestoreConnect(actionType: ActionType, emitPatchFn: (payload) => {}): MethodDecorator {
    return (target, propertyKey: string | Symbol, descriptor: PropertyDescriptor) => {
        ...
        descriptor.value = function() {
            ...
            activeConns.add(
                ...
                originalMethod
                    ...
                    .pipe(
                        takeUntil(
                            race(
                                NgxsInjector.injector
                                    .get(Actions)
                                    .pipe(ofActionCompleted(disconnectAction({ type: action }))),
            ...
            return NgxsInjector.injector.get(Actions).pipe(ofActionCompleted({ type: emitType }), take(1), delay(0));
        };
    };
}
